...
//======//
{
	...
	Term.subTerm = (term, name, value) => {
		if (term[name] !== undefined) {
			throw new Error(`[MotherTode] Sub-term '${name}' is already declared`)
		}
		term[name] = value
		return term
	}
...

{
	...
	scope.MotherTode = Term.args(
		Term.error(
			Term.emit(
				...
				([{output}]) => {
					const lines = []
					lines.push("const global = window")
					lines.push("const scope = {}")
					lines.push("const Term = MotherTode.Term")
					lines.push(`const term = ${output}`)
					lines.push(`term.scope = scope`)
	...
	const makeDefinition = (options = {}, indentSize) => {
		...
		if (exp !== undefined) {
			definition = `Term.export(${definition}, global, "${exp[0]}")`
		}
		return definition
	}
